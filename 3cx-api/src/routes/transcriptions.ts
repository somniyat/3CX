...
export function createTranscriptionsRouter(module: I3CXModule): Router {
  const router = Router();
  ...
  router.get("/", async (req: Request, res: Response) => {
    ...
    const data = await module.getTranscriptions(parsed.data);
  ...
  router.get("/:id", async (req: Request<{ id: string }>, res: Response) => {
    ...
    try {
      const data = await module.getTranscription(id);
      res.json(data);
    } catch (err: any) {
...
export default createTranscriptionsRouter(threecx as unknown as I3CXModule);
